...
- First Fit, Best Fit, Worst Fit에 대해서 설명해 보시오.
  ...
  > First Fit은 사이즈가 n인 프로세스를 메모리에 할당할 때, 크기가 n 이상인 것 중 최초로 찾아지는 hole(가용 공간)에 할당하는 방법이다.  
  > Best Fit은 사이즈가 n인 프로세스를 메모리에 할당할 때, 크기가 n 이상인 것 중 가장 작은 hole(가용 공간)에 할당하는 방법이다. hole들의 리스트가 정렬되어있지 않은 경우 모든 hole을 탐색해야하며, 할당 시 아주 작은 hole들이 여러 개 생성된다는 문제가 있다.  
  > Worst Fit은 사이즈가 n인 프로세스를 메모리에 할당할 때, 크기가 n 이상인 것 중 가장 큰 hole(가용 공간)에 할당하는 방법이다. Best Fit과 마찬가지로 모든 hole을 탐색해야 하며, 상대적으로 아주 큰 hole들이 생성된다.
