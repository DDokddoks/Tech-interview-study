...
- 메모리 관리에서 페이징와 세그멘테이션에 대해서 설명해 보시오.
  > 페이징과 세그멘테이션 모두 불연속적 할당에 해당하는 방법이다.   
  > **페이징(Paging)** 은 logical memory를 동일한 크기의 page들로 나누고 virtual memory의 내용을 page 단위로 불연속적으로 physical memory에 저장하는 것이다. 이때 physical memory는 page와 동일한 크기의 frame들로 나누어지며, 각 page는 그에 해당하는 frame의 위치에 저장된다.   
  > **세그멘테이션(Segmentation)** 은 프로그램을 프로그램의 의미 단위인 Segment들로 나누어 저장하는 것을 말한다. Segment는 작게는 프로그램을 구성하는 함수 하나하나일 수 있고, 크게는 프로그램 전체가 될 수도 있다. 

- First Fit, Best Fit, Worst Fit에 대해서 설명해 보시오.
  > 연속적 할당 방식 중에서 가변분할 방식을 사용할 때 발생할 수 있는 동적 메모리 할당 문제에 대한 대안들이다.   
  > **First Fit** 은 사이즈가 n인 프로세스를 메모리에 할당할 때, 크기가 n 이상인 것 중 최초로 찾아지는 hole(가용 공간)에 할당하는 방법이다.  
  > **Best Fit** 은 사이즈가 n인 프로세스를 메모리에 할당할 때, 크기가 n 이상인 것 중 가장 작은 hole(가용 공간)에 할당하는 방법이다. hole들의 리스트가 정렬되어있지 않은 경우 모든 hole을 탐색해야하며, 할당 시 아주 작은 hole들이 여러 개 생성된다는 문제가 있다.  
  > **Worst Fit** 은 사이즈가 n인 프로세스를 메모리에 할당할 때, 크기가 n 이상인 것 중 가장 큰 hole(가용 공간)에 할당하는 방법이다. Best Fit과 마찬가지로 모든 hole을 탐색해야 하며, 상대적으로 아주 큰 hole들이 생성된다.

- 외부 단편화란? 내부 단편화란?
  > 메모리 할당 시, 사용되지 못하고 메모리 공간이 낭비되는 것을 의미한다.   
  > **내부 단편화(Internal fragmentation)** 는 메모리 상의 분할된 공간이 적재된 프로그램보다 커서 사용되지 못하고 남아있는 공간이다. 고정분할 방식, 페이징에서 발생할 수 있다.  
  > **외부 단편화(External fragmentation)** 는 메모리 상의 분할된 공간이 적재되고자 하는 프로그램보다 작아서 사용되지 못하는 공간이다. 고정분할 방식, 가변분할 방식, 세그멘테이션에서 발생할 수 있다.
  
  ### ⚡️ Chapter 9. Virtual Memory
- 가상 메모리(Virtual Memory)란?  
  > 어떤 프로그램의 크기가 물리 메모리보다 크더라도 **프로그램을 부분적으로 메모리에 올리면서 실행이 가능하다는 점** 을 고려하여, 메모리가 실제 물리 메모리의 크기보다 더 많은 것처럼 보이게 만드는 기술이다. 

- 가상 메모리를 사용할 시 장단점은? 
  > **물리 메모리 크기의 제약을 받지않고** 매우 큰 가상 주소 공간을 가정하고 큰 프로그램을 만들 수 있다.  
  > 각 프로그램이 더 작은 메모리를 차지하므로 더 많은 프로그램을 동시에 실행할 수 있어 **병행성과 CPU 이용률, 처리율이 증가**한다. 
  > 반복적으로 page fault가 발생하게 되면 프로세스 입출력에 대부분의 시간이 낭비되는 **thrashing**이 발생할 수 있다. 

- Demand Paging이란?  
  > 실제로 필요할 때 해당하는 page를 메모리에 올리는 것이다. 대부분의 프로그램은 아주 일부분만 빈번하게 사용되고 나머지는 그렇지 않으므로 프로그램의 전체가 메모리에 항상 있을 필요없이 필요할 때 필요한 부분만 메모리에 올리면서 실행하는 것이 효율적이다. 

- page fault란?
  > 필요한 page가 메모리에 올라와 있지 않아 backing store에서 page를 찾아 메모리에 올려놓아야 하는 상황을 의미한다.

- 페이지 교체란?  
  > 필요한 page가 메모리에 없으면서 메모리가 다른 페이지로 가득차 공간이 없을 때, 메모리 상의 어떤 페이지를 쫓아낼 지 결정하고 그 자리에 필요한 page를 올리는 것을 말한다.

- 페이지 교체 알고리즘의 종류와 각각의 특징에 대해 설명해 보시오.
  > - **Belady's Optimal Algorithm**   
  >   - 가장 먼 미래에 참조되는 page를 교체하는 알고리즘이다.   
  >   - 실제로 사용되는 것이 아니라 미리 정해진 일련의 input에 대해 실행하는 알고리즘(offline algorithm)으로, 최적의 성능을 보이는 알고리즘이기 때문에 다른 페이지 교체 알고리즘의 성능 상한선 기준이 되는 알고리즘이다.   
  > - **FIFO Algorithm**  
  >   - 가장 먼저 들어온 page를 교체하는 알고리즘이다.   
  >   - page frame이 많아질수록 page fault가 늘어나는 특징이 있다.  
  > - **LRU Algorithm**      
  >   - 가장 오래 전에 참조된 page를 교체하는 알고리즘이다.  
  >   - 교체할 페이지를 선택하는데 있어서 O(1)의 성능을 보인다.  
  >   - 과거의 참조 빈도를 고려하지 못한다는 특징이 있다.  
  > - **LFU Algorithm**    
  >   - 교체 시점에 가장 참조 횟수가 적은 page를 교체하는 알고리즘이다.   
  >   - 교체할 페이지를 선택하는데 있어서 O(logN)의 성능을 보인다.   
  >   - LRU 에 비해 page들의 과거부터 현재까지의 참조 빈도(인기도)를 반영할 수 있다는 장점이 있지만 반대로 참조 시점의 최근성을 반영하지 못하고 구현이 복잡하다는 특징이 있다.  
  > - **Clock Algorithm**    
  ...
  >   - Second Chance Alogrithm 이라고도 불리며, 최근에 참조되지 않은 page를 교체하는 방식의 알고리즘이다. page들로 구성된 Circular list를 차례대로 지속적으로 탐색하며 Reference bit(해당 페이지가 참조될 때 하드웨어에 의해 1이 되는 bit)를 체크하는데, 1이면 0으로 교체한 후 다음으로 이동하고, 0이면 그 페이지를 교체한다.  
  >   - 지속적으로 페이지들의 최근 참조 여부를 체크하기 때문에 LRU Algorithm을 사용할 때와 유사한 효과를 낼 수 있다.  
  >   - Modified bit(해당 페이지의 내용이 변경되었을 때 1이 되는 bit)를 추가하여 개선할 수 있다. 교체하고자 하는 page의 Modified bit가 1이면 그 page를 쫓아내기 전에 backing store에 변경된 내용을 반영하고 쫓아내야 하므로 할 일이 많아지기 때문에, modified bit 또한 0인 page를 우선적으로 선택한다.
