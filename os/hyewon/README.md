...
- 메모리 관리에서 페이징와 세그멘테이션에 대해서 설명해 보시오.
  ...
  > Paging은 프로세스의 주소 공간을 페이지(page)란 단위의 고정된 사이즈로 나누어 물리적 메모리에 불연속으로 저장하는 방식이다. 프로세스는 page라는 고정 크기로 분할되고, 메모리는 frame이라는 고정 크기로 분할된다.   
  > Segmentation은 물리적인 고정 크기로 분할하는 페이징과는 달리 프로그램을 의미 단위(논리적 단위)인 여러 개의 segment로 구성해 프로세스의 메모리 공간을 분리하는 기법이다. 일반적으로 code, data, stack 부분이 하나씩의 segment로 정의된다.   
  > Segmentation은 보호와 공유에서 효율적이고, Paging은 외부 단편화 문제를 해결할 수 있다. 두 장점을 합쳐 세그먼트를 페이징 기법으로 나누는 Paged Segmentation 방식도 존재한다.
  
- First Fit, Best Fit, Worst Fit에 대해서 설명해 보시오.

  > 메모리의 가변 분할 방식에서 size n인 요청을 만족하는 가장 적절한 hole을 찾는 방법이다.  
  > First-fit : size가 n 이상인 것 중 최초로 찾아지는 hole에 할당한다.  
  > Best-fit : size가 n 이상인 가장 작은 hole을 찾아서 할당하는 방식으로, 많은 수의 아주 작은 hole이 생성된다.  
  > Worst-fit : 가장 큰 hole에 할당하는 방식으로, 상대적으로 아주 큰 hole이 생성된다.  
  > First-fit과 Best-fit이 Worst-fit보다 속도와 공간 이용률 측면에서 효과적이다.  

- 외부 단편화란? 내부 단편화란?

  > External Fragmentation(외부 단편화) : 프로그램의 크기보다 파티션의 크기가 작은 경우 해당 파티션이 비어있는 데도 불구하고 프로그램을 적재하지 못하기 때문에 생기는 메모리 공간을 의미한다.  
  > Internal Fragmentation(내부 단편화) : 각 프로그램의 크기보다 파티션의 크기가 큰 경우 해당 파티션에 프로그램을 적재하고 남는 메모리 공간을 의미한다.
...
- 가상 메모리(Virtual Memory)란?  
  ...
  > 가상 메모리(Virtual Memory)는 물리적인 메모리의 크기 한계를 극복하기 위한 메모리 관리 기법의 하나로, 실제의 물리 메모리 개념과 사용자의 논리 메모리 개념을 분리한 것이다. 즉, 프로그램에 실제 메모리 주소가 아닌 가상의 메모리 주소를 주는 방식이다.

- 가상 메모리를 사용할 시 장단점은?  

  > 장점
  > - 실제 메모리 크기에 제약받지 않는다.
  > - SWAP에 필요한 입출력이 줄어들어 실행이 빨라진다.(context switch, kernel 개입 최소화)
  > - CPU 이용률과 처리율이 높아진다.  
  
  > 단점
  > - 오히려 컴퓨터 속도가 낮아질 수 있다.
  > - 디스크의 수명이 짧아질 수 있다.
  
- Demand Paging이란? 

  > 프로세스에서 사용할 메모리 page를 프로세스 생성 시에 모두 할당하지 않고, 필요할 때마다 동적으로 할당하는 기법이다. MMU를 사용해서 여러 프로세스가 시스템의 메모리를 효율적으로 공유할 수 있게 한다.

- page fault란?  
  > Demand Paging 방식을 사용할 때 CPU가 프로그램을 실행하면서 필요한 페이지가 물리적 메모리에 없는 경우를 의미한다.

- 페이지 교체란?  
  > 메모리가 꽉 찬 상태(Free frame이 없는 상태)에서 page fault 발생 시, 메모리에 있는 페이지 중 가장 쓸모 없는 것을 골라 쫓아내는 것을 의미하고, 이때 어떤 것을 쫓아낼지 결정해야 한다.

- 페이지 교체 알고리즘의 종류와 각각의 특징에 대해 설명해 보시오.  
  > **1. OPT(Optimal/MIN) Algorithm**   
  > 물리적 메모리에 존재하는 페이지 중 가장 먼 미래에 참조될 페이지를 쫓아내는 알고리즘으로, 실제로는 미래의 참조되는 페이지를 알지 못해 사용할 수 없다(Offline algorithm).  
  > **2. FIFO(First-In-First-Out) Algorithm**  
  > 물리적 메로리에 가장 먼저 올라온 페이지를 우선적으로 쫓아내는 알고리즘이다. 메모리를 증가하였음에도 page fault가 오히려 늘어나는 현상(FIFO Anomaly)이 발생할 수 있다.  
  > **3. LRU(Least Recently Used) Algorithm**  
  > 가장 오래 전에 참조가 이루어진 페이지를 쫓아내는 알고리즘이다. 이는 시간지역성(temporal locality, 최근에 참조된 페이지가 가까운 미래에 다시 참조될 가능성이 높은 성질)를 이용한 알고리즘으로 실제로 가장 많이 사용된다. LinkedList 방식으로 구현 가능하다.  
  > **4. LFU(Least Frequently) Algorithm**  
  > reference count(참조 횟수)가 가장 적은 페이지를 교체하는 알고리즘으로, LRU처럼 직전 참조 시점만 보는 것이 아니기 때문에 page의 인기도를 좀 더 정확히 반영할 수 있으나, 참조 시점의 최근성을 반영하지 못하고 LRU보다 구현이 어렵다(Heap).  
  > **5. Clock(NUR(Not Used Recently)/Second chance) Algorithm**  
  > LRU의 근사 알고리즘으로 운영체제는 가장 오래 전에 참조된 페이지를 알 수 없으므로, 대신 최근에 사용되지 않는 페이지를 쫓아내는 방식이다(circular list). 페이지 프레임의 reference bit를 조사해서 값이 0인 페이지를 찾으면 그 페이지와 교체한다.    
